...
class FakeRunner(QObject):
    ...
    def request_configuration_load(self) -> None:
        ...
        QTimer.singleShot(
            0,
            lambda: self.configuration_ready.emit(default_configuration_snapshot()),
        )
    ...
    def request_global_save(self, settings: GlobalSettings) -> None:
        ...
        QTimer.singleShot(
            0,
            lambda: self.configuration_ready.emit(
                default_configuration_snapshot().__class__(
...
def test_load_save_and_restart_flag() -> None:
    ...
    assert _wait_until(lambda: controller.current_snapshot is not None)
    # Already zh-CN after first save response path uses pending_locale only on change
    # from previous snapshot locale.
...
def test_provider_save_test_and_failure_retention() -> None:
    ...
    runner.configuration_failure.emit(RunnerFailure(code="config.write_failed"))
    assert _wait_until(lambda: controller.last_failure is not None)
    assert controller.current_snapshot is previous
