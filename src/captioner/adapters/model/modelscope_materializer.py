"""ModelScope fixed-revision materializer with transaction-local cache."""
...
class ModelScopeModelMaterializer(ModelMaterializer):
    def __init__(
        self,
        ...
        *,
        ...
        if config.source_id != "modelscope":
    ...
    def materialize(
        ...
        *,
        ...
        if reference.source_id != "modelscope" or not reference.revision:
        ...
        cache = destination / ".modelscope-cache"
        cache.mkdir(parents=True, exist_ok=True)
        _emit(progress, "resolving_source")
        old_cache = os.environ.get("MODELSCOPE_CACHE")
        old_modules = os.environ.get("MODELSCOPE_MODULES_CACHE")
        try:
            os.environ["MODELSCOPE_CACHE"] = str(cache)
            os.environ["MODELSCOPE_MODULES_CACHE"] = str(cache / "modules")
            downloader = self._downloader()
            _emit(progress, "downloading")
            with redirect_stdout(sys.stderr):
                downloader(
                    model_id=reference.repository_id,
                    ...
                    allow_patterns=list(
                        ...
                        else MODELSCOPE_CT2_ALLOW_PATTERNS
                    ),
                    ignore_patterns=list(_MODELSCOPE_IGNORED_PATTERNS),
            ...
            _restore_environment("MODELSCOPE_CACHE", old_cache)
            _restore_environment("MODELSCOPE_MODULES_CACHE", old_modules)
    ...
    def _downloader(self) -> _SnapshotDownload:
        if self._snapshot_download is None:
            try:
                module = import_module("modelscope")
